 ...
 * RentItemPage
 * ------------
 * Frontend (React) page for a seller listing an item for RENT.
 * Drops into your MERN app's src/pages or src/components folder.
 *
 * Wire up to your Express + MongoDB backend at the TODO spot in handleSubmit:
 *   POST /api/listings   (multipart/form-data, since images are uploaded as files)
 *
 * Security deposit rule (business logic, enforced identically on the backend
 * when you build the booking flow): deposit = 50% of (rentRate * numberOfDays).
 * e.g. Rs.500/day x 4 days = Rs.2000 rent, Rs.1000 deposit (50%), Rs.3000 total.
 * On this listing page the seller only sets the daily rate; the day-count and
 * final deposit are calculated per booking. The "Estimate" box below just
 * previews that math for the seller using a sample day count.
 *
 ...
 *     images: { type: [String], required: true },   // stored URLs, min 1
 *     rentRate: { type: Number, required: true },    // per day
 *     rentUnit: { type: String, enum: ["day", "week", "month"], default: "day" },
 *     depositPercent: { type: Number, default: 50 }, // used to compute deposit per booking
 *     condition: { type: String, required: true },
 *     pickupOption: { type: String, enum: ["shop", "other"], required: true },
 *     shopName: String,
 ...
 *   }, { timestamps: true });
 *
 *   // Booking-time calculation (do this again server-side, never trust the client):
 *   const totalRent = rentRate * numberOfDays;
 *   const securityDeposit = totalRent * (depositPercent / 100);
...
export default function RentItemPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // data.append("deliveryAvailable", deliveryAvailable);
      // data.append("depositPercent", 50);
      // await axios.post("/api/listings", data, {
      //   headers: { "Content-Type": "multipart/form-data" },
      // });

      await new Promise((res) => setTimeout(res, 700)); // fake latency
      setSavedMsg("Listing posted.");
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Scrollable body, centered with a readable max width on big screens */}
        <div className="flex-1 overflow-y-auto">
          <div className="px-6 sm:px-10 lg:px-16 py-8 space-y-7 max-w-3xl mx-auto">
            ...
            {/* Photos - required, gallery + camera */}
            <div>
              ...
              />
              {/* Camera capture - opens the device camera directly on mobile */}
              <input
                ref={cameraInputRef}
                type="file"
                accept="image/*"
